                            ...
                            db.collection("Items").document(document.documentID).collection("user-items")
                                .addSnapshotListener { querySnapshot, error in
                                    ...
                                    itemList = documents.map { queryDocumentSnapshot -> UserItem in
                                        ...
                                        let _price = data["Price"] as? String ?? "0.0"
                                        
                                        return UserItem(id: _id, owner: document.documentID, name: _name, dateFrom: _dateFrom, dateUntil: _dateUntil, category: _category, price: _price, quantity: _quantity, unit: _unit, location: _location, locationName: _locationName, description: _description)
                                    }
                                    
                                    for item in itemList {
                                        print("<Name: \(item.getname()) \t Id: \(item.getID()) \t dateFrom: \(item.getValidFromDate()) \t dateUntil: \(item.getValidUntilDate()) \t lat: \(item.getLocationLatitude()) \t long: \(item.getLocationLongitude()) \t <Price>: \(item.getPrice()) \n")
                                    }
                                    //}
                                }}
                    }
